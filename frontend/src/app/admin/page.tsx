...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-gray-50 flex flex-col md:flex-row">
      {/* Sidebar - Antd Style */}
      <aside className="w-full md:w-64 bg-[#001529] text-white shrink-0 md:min-h-[calc(100vh-64px)] flex flex-col transition-all">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="bg-white px-6 py-4 border-b border-gray-200 flex items-center justify-between shrink-0 shadow-sm z-10">
          <h1 className="text-xl font-extrabold text-gray-800 flex items-center gap-2">
             {menuItems.find(m => m.id === activeTab)?.icon} 
             {menuItems.find(m => m.id === activeTab)?.label}
          </h1>
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold text-gray-600">Xin chào, {user?.fullName || 'Quản trị viên'}</span>
