...
import { ChartPieSlice, Books, ShieldCheck, UsersThree, SlidersHorizontal, ArrowSquareOut, House, Sparkle } from '@phosphor-icons/react';
...
export function AdminSidebar() {
  ...
  const baseMenuItems = [
    {
      ...
      icon: ChartPieSlice,
      badge: 'Real-time',
      roles: ['ADMIN', 'CONTENT_MANAGER', 'LECTURER'],
    },
    {
  ...
  // Filter items based on user role (fallback to ADMIN if unknown)
  const menuItems = baseMenuItems.filter((item) => {
    const allowed = item.roles ?? [];
    return allowed.includes(user?.role ?? 'ADMIN');
  });

  return (
    <aside
      className={`fixed inset-y-0 left-0 z-50 flex flex-col border-r border-slate-800 bg-slate-950 text-slate-200 transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'}`}
    >
      {/* Brand Header */}
