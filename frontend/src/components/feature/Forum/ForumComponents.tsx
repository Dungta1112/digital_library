...
export function CommentItem({ comment, onDelete }: { comment: ForumComment; onDelete?: (commentId: string) => void }) {
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl border border-slate-200/80 dark:border-slate-800 mb-4 shadow-sm hover:shadow-md transition-all relative">
            {can('MODERATE_FORUM') && (
                <div className="absolute top-4 right-4">
                    <button onClick={() => onDelete?.(comment.id)} className="flex items-center gap-1 text-[11px] font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2.5 py-1.5 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors active:scale-95">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-0.5">
                            ...
                            <span className={`text-[10px] px-2 py-0.5 rounded-md font-bold uppercase tracking-wider transition-colors duration-300 ${comment.authorRole === 'LECTURER' || comment.authorRole === 'FACULTY'
