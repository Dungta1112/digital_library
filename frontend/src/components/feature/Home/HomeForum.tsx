        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {loading ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="h-64 rounded-3xl bg-white border border-gray-100 shadow-sm animate-pulse" />
            ))
          ) : (
            posts.map((post, index) => (
              <motion.div
                ...
                whileHover={{ y: -5 }}
                className="p-8 rounded-3xl bg-white border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col"
              >
                ...
                <Link href={`/forum/post/${post.id}`} className="w-full text-center py-3 rounded-xl bg-gray-50 text-gray-700 font-medium hover:bg-green-50 hover:text-green-700 transition-colors mt-auto text-sm block">
                  Tham gia thảo luận
                </Link>
              </motion.div>
            ))
