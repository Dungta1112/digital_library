        ...
        return (
            <div className="flex-1 flex items-center justify-center bg-white dark:bg-slate-950 transition-colors duration-300">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-sm text-gray-400 dark:text-gray-500">Đang tải tin nhắn...</span>
                </div>
            </div>
        );
    }
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-4 transition-colors duration-300">
                {messages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center">
                        ...
                        <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-1">Bắt đầu cuộc trò chuyện</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xs">Hãy là người đầu tiên gửi tin nhắn trong kênh này.</p>
                    </div>
                ) : (
                                            ...
                                            <div className="flex items-baseline gap-2 mb-0.5">
                                                ...
                                                <span className="text-[11px] text-gray-400 dark:text-gray-500 font-medium">
                                                    {new Date(msg.timestamp).toLocaleDateString() === new Date().toLocaleDateString()
                                                        ? `Hôm nay ${new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`
            ...
            <div className="px-4 pb-5 pt-2 bg-white dark:bg-slate-950 transition-colors duration-300">
                <form onSubmit={handleSendMessage} className="relative flex items-center">
                    ...
                    <Input
                        ...
                        placeholder={user ? "Nhắn tin trong #chung" : "Đăng nhập để gửi tin nhắn"}
