        ...
        <Document
          ...
          error={
            <div className="rounded bg-red-500/20 p-10 text-white">
              Failed to load PDF file. Check that the file URL exists.
            </div>
          }
        >
          <Page
  ...
  useEffect(() => {
    ...
    async function loadDocx() {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Cannot load DOCX file (${response.status})`);
        }
        ...
        if (!cancelled) {
          setError(loadError instanceof Error ? loadError.message : 'Cannot read DOCX file.');
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-gray-200 p-3">
        <span className="text-sm font-semibold text-gray-700">DOCX Preview</span>
      ...
      <div className="flex-grow overflow-auto bg-slate-100 p-4">
        <div className="mx-auto min-h-full max-w-3xl rounded-lg bg-white p-8 shadow-sm">
          {loading && <p className="text-sm text-slate-500">Loading DOCX...</p>}
          {error && <p className="text-sm text-red-600">{error}</p>}
          {!loading && !error && paragraphs.length === 0 && (
            <p className="text-sm text-slate-500">No readable text found in this DOCX file.</p>
          )}
...
function UnsupportedDocumentViewer({ url }: { url: string }) {
  return (
    <div className="flex h-full min-h-[600px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center">
      <h2 className="mb-2 text-lg font-bold text-slate-900">Unsupported document type</h2>
      <p className="mb-6 max-w-md text-sm text-slate-500">
        This viewer supports PDF and DOCX files. You can still open or download this file directly.
      </p>
